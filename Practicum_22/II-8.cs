...
class Program // Объявляем класс программы
{
    static void Main(string[] args) // Главная функция программы
    {
        ...
        if (IsEulerian(adjacencyMatrix, n)) // Проверяем, является ли граф эйлеровым
        {
            // Запускаем эйлеров цикл без вывода
            ExecuteEulerCycle(adjacencyMatrix, n);
    ...
    // Функция для выполнения Эйлерова цикла без вывода
    static void ExecuteEulerCycle(int[,] matrix, int n)
    {
        // Копируем матрицу смежности, чтобы не изменять исходную
        int[,] tempMatrix = (int[,])matrix.Clone();
        Stack<int> stack = new Stack<int>();
        int currentVertex = 0; // Начинаем с первой вершины
        stack.Push(currentVertex);

        while (stack.Count > 0)
        {
            if (HasEdges(tempMatrix, currentVertex, n))
            {
                stack.Push(currentVertex);
                for (int i = 0; i < n; i++)
                {
                    ...
                    {
                        tempMatrix[currentVertex, i]--;
                        tempMatrix[i, currentVertex]--;
                        currentVertex = i;
                        break;
                    }
                }
    ...
    // Функция для проверки наличия ребер у вершины
    static bool HasEdges(int[,] matrix, int vertex, int n)
    {
        for (int i = 0; i < n; i++)
        {
